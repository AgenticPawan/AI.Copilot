...
Generate EF Core migration for schema changes in VirtualStudio.
...
Check `src/backend/VirtualStudio.Persistence/Configurations/EntityConfigurations.cs`:
...
Update seeders in `src/backend/VirtualStudio.Persistence/Seeders/` if the new entity needs initial data.
...
# Or via EF CLI
dotnet ef database update --context {Context} --project VirtualStudio.Persistence --startup-project VirtualStudio.Api
```
