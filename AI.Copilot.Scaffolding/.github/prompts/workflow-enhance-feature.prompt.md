...
Add new capabilities to an existing feature in {{projectName}} without breaking current functionality.
...
1. Read the existing files across all layers:
   - Entity: `{{projectName}}.Domain/Entities/{Entity}.cs`
   - DTOs: `{{projectName}}.Application/DTOs/{Area}/`
   - Handlers: `{{projectName}}.Application/Features/{Area}/`
   - Controller: `{{projectName}}.Api/Controllers/{Area}/`
