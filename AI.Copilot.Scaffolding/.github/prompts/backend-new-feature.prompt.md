---
agent: agent
description: "Add a new backend feature end-to-end. Use when: creating a new entity with full CRUD (Entity → DTO → Command/Query → Handler → Validator → DbContext → Controller → Permissions → Migration). Generates production-ready code following {{projectName}} Clean Architecture patterns."
---

# Backend Feature: Add New Entity & CRUD

You are building a new backend feature for the {{projectName}} multi-tenant SaaS platform.
...
Create `src/backend/{{projectName}}.Domain/Entities/{Entity}.cs`:
...
### Step 2: DTOs
Create `src/backend/{{projectName}}.Application/DTOs/{Entity}/{Entity}Dtos.cs`:
...
Create in `src/backend/{{projectName}}.Application/Features/{Entity}/`:
...
### Step 4: Validators
Create `src/backend/{{projectName}}.Application/Features/{Entity}/Validators/{Entity}Validators.cs`:
...
Add to the appropriate interface in `src/backend/{{projectName}}.Application/Interfaces/`:
...
Add to `src/backend/{{projectName}}.Persistence/Configurations/EntityConfigurations.cs`:
...
Create `src/backend/{{projectName}}.Api/Controllers/{Area}/{Entities}Controller.cs`:
...
```bash
cd src/backend
dotnet ef migrations add Add{Entity} --project {{projectName}}.Persistence --startup-project {{projectName}}.Api --context {Host|Tenant}DbContext --output-dir Migrations/{Host|Tenant}
```
